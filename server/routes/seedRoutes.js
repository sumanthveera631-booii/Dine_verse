...
// @route   POST /api/seed
// @desc    Seed database with menu items and venues
// @access  Public (for first-time setup only)
  ...
  try {
    ...
    if (menuCount > 0 || venueCount > 0) {
      return res.status(400).json({ 
        message: 'Database already seeded. Clear manually if you want to reset.' 
      });
    }
