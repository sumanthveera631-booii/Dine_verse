...
// Enforce configuration environment variables if local
process.env.MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/dineverse';
...
const menuItemsData = [
  ...
  },
  {
    ...
    description: 'Cold-pressed dark pomegranate juice, cedar wood smoke injection, fresh lime zest, and mineral mineral water spritz.',
    price: 26,
    category: 'Mocktails',
...
const privateRoomsData = [
  {
    ...
    features: ['Private Entrance', 'Dedicated Waitstaff', 'Cloaked Sound System']
  },
  {
    ...
    features: ['Retractable Roof', 'Private Bar', 'Sky Lighting']
  },
  {
    ...
    features: ['Wine Cellar', 'Private Sommelier', 'Secure Access']
  },
  {
    ...
    features: ['Antique Decor', 'Personal Butler', 'Floral Arrangements']
  },
  {
    ...
    features: ['Biophilic Climate', 'In-house Botanist', 'Seasonal Menu']
  },
  {
    ...
    features: ['Chef Preview', 'Private Kitchen Access', 'Custom Lighting']
  }
];
...
const seedDB = async () => {
  try {
    ...
    console.log(`✅ Success: Seeded ${insertedMenu.length} Luxury Menu Items.`);

    console.log('🌱 Seeding Private Venue Rooms...');
    const insertedRooms = await PrivateRoom.insertMany(privateRoomsData);
    console.log(`✅ Success: Seeded ${insertedRooms.length} Private Rooms.`);

    console.log('🌱 Seeding Standard Credentials...');
    
    // Seed standard test user
    const standardUser = await User.create({
      name: 'Julian Sterling',
      email: 'user@dineverse.com',
      password: 'user123',
      role: 'user',
      wishlist: [insertedMenu[0]._id, insertedMenu[4]._id, insertedMenu[8]._id]
    });
    console.log('✅ Created User: user@dineverse.com (password: user123)');

    // Seed admin test user
    const adminUser = await User.create({
      name: 'Imperia Concierge',
      email: 'admin@dineverse.com',
      password: 'admin123',
      role: 'admin',
    });
    console.log('✅ Created Admin: admin@dineverse.com (password: admin123)');
